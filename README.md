...
- **首页宇宙**：发布记录 63,025 部，覆盖封面与引言、主叙事（格局 / 发现 / 拐点 / 对照 / 三波浪潮 / 五维）及终章回响。序章可视点约 10,800 颗陆地粒子，封面另加约 7,500 颗尘埃，不是 6 万颗同时画满。
- **封面 / 引言**：共用 `universe` 场景。飞入、散开、地区聚拢走 `#prologue-motion-layer` Canvas（圆点、按色分桶；小点在 1× 缓冲上绘制再贴到屏幕）；停稳后隐藏的 ECharts 层负责点选与 tooltip。停留时粒子按 `visualPhase` / `visualFreq` 缓慢错开呼吸；系统「减少动态」时仍是静止散点。
...
四个附录分卷（年代变化 / 地区分布 / 语言构成 / 交叉筛选）已从正式前端下线。首页终章探索舱覆盖交叉筛选；分卷作为独立副本归档至 [`archive/vol1_time.html`](archive/vol1_time.html) 等，通过本地服务器打开（`python serve.py` 后访问 `http://127.0.0.1:8000/archive/vol1_time.html`）。源码与冻结打包产物在 [`archive/volumes/`](archive/volumes/)，不参与 `npm run build`。`.volume-page` 样式仍留在 `frontend/style.css`，归档页通过主站 hashed CSS 读取。

主站 JS（ECharts 主包 + `app`）gzip 预算为 300KB；当前约 238KB（以 `npm run check:budget` 为准）。
...
| `data/frontend_dataset.json` | 首页和探索舱使用的紧凑浏览器载荷 |
...
│   ├── js/                           数据服务、叙事和图表源码（序章运动层：js/scenes/prologue.js）
│   ├── src/                          ECharts 主包与首页入口
│   ├── style.css                     首页样式（含归档分卷仍复用的 `.volume-page`）
