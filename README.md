...
- **首页宇宙**：63,025 颗全屏电影粒子与序章、分幕叙事（第四幕含时间坡度上下两步）及终章回响的滚动叙事。
- **每幕数据卡**：筛选、均值、中位数、标准差、四分位数、高分占比和随机打捞。
- **比较辅助线**：横向统计基准、纵向组别或年份定位线，以及差距色带。
...
- 63,025 条发布记录；
- 64 个详情分片；
- 59 项数据、叙事和页面测试通过；冻结校验含 geo / story_universe 指纹；
...
- 年份切分滑杆：`century-decline` 幕将分界年上限定为 2020（`frontend/js/app.js` 硬编码 `max: 2020`），并非当年样本已耗尽。
- 时间覆盖：2026-08-18 合并 delivery_20260817 后覆盖至 2026 年；2022 年后共 7,190 部，以北美/欧洲/东亚为主，中国（含港澳台）占 15.4%；2020s 中国方言片仅 57 部，该年代结论谨慎引用。
...
- 数据没有按全球电影总体进行概率抽样。
- 地区取第一制片国家／地区，合拍片会被归入单一分析地区；「中国」组含香港、台湾、澳门——方言片构成：港 2,487／台 179／大陆 374／澳门 5。大陆（含澳门）子样本敏感性：方言 7.05／烂片率 4.2%（n=379）vs 普通话 5.98／28.9%（n=7,219），方向与主口径一致。
- 主类型取类型列表第一项。
